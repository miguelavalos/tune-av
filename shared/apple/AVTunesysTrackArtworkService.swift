...
struct AVTunesysTrackArtwork: Equatable, Sendable {
    let albumTitle: String?
    let artworkURL: URL?
    let source: String
}

actor AVTunesysTrackArtworkService {
    private let session: URLSession
    private let userAgent: String

    init(session: URLSession = .shared, userAgent: String = "AVTunesys/0.1") {
        self.session = session
        self.userAgent = userAgent
    }

    func resolveArtwork(artist: String, title: String) async -> AVTunesysTrackArtwork? {
        ...
        do {
            ...
            let payload = try JSONDecoder().decode(AVTunesysITunesSearchResponse.self, from: data)
    ...
    private func matchScore(_ item: AVTunesysITunesTrack, artist: String, title: String) -> Int {
...
private struct AVTunesysITunesSearchResponse: Decodable {
    let results: [AVTunesysITunesTrack]
}

private struct AVTunesysITunesTrack: Decodable {
