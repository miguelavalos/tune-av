...
extension Station {
    ...
    var flagEmoji: String? {
        guard let code = AVTunesysCountry.sanitizedCode(countryCode) else { return nil }
        return AVTunesysCountry(code: code, name: code).flag
    }
    ...
    var resolvedHomepageURL: URL? {
        guard let homepageURL = AVTunesysText.normalizedValue(homepageURL) else {
            return nil
        }
    ...
    var shareText: String {
        if let homepageURL = AVTunesysText.normalizedValue(homepageURL) {
            return "\(name)\n\(homepageURL)"
        }
    ...
    func cardDetailText(
        ...
        let normalizedLanguage = AVTunesysText.normalizedValue(language, excluding: unknownValues, locale: locale)
        let normalizedCountry = AVTunesysText.normalizedValue(country, excluding: unknownValues, locale: locale)
        let normalizedState = AVTunesysText.normalizedValue(state, excluding: unknownValues, locale: locale)
    ...
    func hasResolvedCountry(unknownCountryValues: [String], locale: Locale = .current) -> Bool {
        if AVTunesysCountry.sanitizedCode(countryCode) != nil {
            return true
        }

        return AVTunesysText.normalizedValue(country, excluding: unknownCountryValues, locale: locale) != nil
    }
}
