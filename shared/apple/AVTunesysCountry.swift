...
struct AVTunesysCountry: Identifiable, Hashable {
    let code: String
    let name: String
    ...
    static func all(localizedName: (String) -> String) -> [AVTunesysCountry] {
        Locale.Region.isoRegions
            .compactMap { region -> AVTunesysCountry? in
                guard let code = sanitizedCode(region.identifier) else { return nil }
                return AVTunesysCountry(code: code, name: localizedName(code))
            }
            .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
    }

    static func filtered(_ options: [AVTunesysCountry], query: String) -> [AVTunesysCountry] {
        guard let normalizedQuery = AVTunesysText.normalizedValue(query) else {
            return options
        }
