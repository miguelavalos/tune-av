...
enum AVTunesysEighties80sNowPlaying {
    ...
    static func parseTrack(for station: Station, from html: String) -> AVTunesysNowPlayingTrack? {
        let entries = parseEntries(from: html)
        guard let entry = bestEntry(for: station, in: entries) else { return nil }
        return AVTunesysNowPlayingTrack(title: entry.songTitle, artist: entry.artistName)
    }
