...
- **local library mode**: the product-facing storage mode where user-authored Tune AV library data stays on the device even though catalog, Avi, or account-backed services may be read from the backend.
- **connected account**: the product-facing mode for a signed-in free user. Identity is connected and backend reads may be available, but Tune AV library sync remains local-only unless capabilities say otherwise.
...
- **capability**: a specific permission derived from entitlement, such as backend service use, cloud sync, premium features, or plan management.
