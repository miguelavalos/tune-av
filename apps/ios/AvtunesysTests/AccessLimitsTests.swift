...
final class AccessLimitsTests: XCTestCase {
    func testAccessPolicyMatchesSharedContract() throws {
        ...
        XCTAssertEqual(contract.appId, "avtunesys")
        ...
        for expectedMode in expectedModes {
            ...
            XCTAssertEqual(AccessCapabilities.forMode(expectedMode.mode), contractMode.capabilities.avtunesysValue)
            XCTAssertEqual(AccessLimits.forMode(expectedMode.mode), contractMode.limits.avtunesysValue)
        }
    }
    ...
    func testLibrarySyncPlannerPushesLocalWhenRemoteIsEmpty() {
        ...
        XCTAssertEqual(
            AVTunesysLibrarySyncPlanner.decision(
                localSnapshot: local,
    ...
    func testLibrarySnapshotMergerKeepsFavoritesFromBothDevices() {
        ...
        let merged = AVTunesysLibrarySnapshotMerger.merged(local: local, remote: remote)
...
private struct AccessCapabilitiesContract: Decodable {
    ...
    var avtunesysValue: AccessCapabilities {
        AccessCapabilities(
...
private struct AccessLimitsContract: Decodable {
    ...
    var avtunesysValue: AccessLimits {
        AccessLimits(
