...
struct AppShellSearchRequest: Equatable {
    ...
    init(query: String, tag: String?, countryCode: String?) {
        self.query = AVTunesysText.normalizedValue(query) ?? ""
        self.tag = AVTunesysText.normalizedValue(tag)
        self.countryCode = AVTunesysCountry.sanitizedCode(countryCode)
    }
...
struct AppShellSearch {
    ...
    static func orderedDiscoveryCountryCodes(
        ...
        for code in seedCountryCodes.compactMap(AVTunesysCountry.sanitizedCode) where seenCodes.insert(code).inserted {
