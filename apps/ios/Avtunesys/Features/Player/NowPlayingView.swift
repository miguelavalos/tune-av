...
struct NowPlayingView: View {
    ...
    private var playerBackdrop: some View {
        ZStack {
            AvtunesysTheme.onboardingBackground.ignoresSafeArea()

            Circle()
                .fill(
                    RadialGradient(
                        colors: [AvtunesysTheme.highlight.opacity(0.20), .clear],
                        center: .center,
                        startRadius: 0,
                        endRadius: 180
                    )
            ...
            Circle()
                .fill(
                    RadialGradient(
                        colors: [AvtunesysTheme.highlight.opacity(0.12), .clear],
                        center: .center,
                        startRadius: 0,
                        endRadius: 150
                    )
    ...
    private func artworkShowcase(for station: Station, size: CGFloat) -> some View {
        FlippingPlayerArtwork(
            ...
            .background {
                Circle()
                    .fill(AvtunesysTheme.highlight.opacity(0.22))
    ...
    private func transportSection(contentWidth: CGFloat, compact: Bool) -> some View {
        ...
        return HStack(spacing: compact ? 14 : 18) {
            ...
            Button {
                ...
                ZStack {
                    Circle()
                        .fill(AvtunesysTheme.signalGradient)
                        .shadow(color: AvtunesysTheme.highlight.opacity(0.25), radius: 18, y: 10)
    ...
    private var loadingStatusPill: some View {
        HStack(spacing: 8) {
            ProgressView()
                .tint(AvtunesysTheme.textInverse.opacity(0.86))
                .controlSize(.small)

            Text(L10n.string("audio.status.loading"))
        }
        .font(.caption.weight(.semibold))
        .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.86))
        .padding(.horizontal, 12)
        .padding(.vertical, 8)
        .background(AvtunesysTheme.highlight.opacity(0.24), in: Capsule())
        .overlay {
            Capsule()
                .stroke(AvtunesysTheme.highlight.opacity(0.28), lineWidth: 1)
        }
    ...
    private func statusPill(text: String) -> some View {
        Text(text)
            .font(.caption.weight(.semibold))
            .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.82))
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
    ...
    private func optionsMenu(for station: Station) -> some View {
        Menu {
            ...
            Image(systemName: "ellipsis")
                ...
                .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.78))
    ...
    private func favoriteButton(for station: Station) -> some View {
        Button {
            ...
            Image(systemName: libraryStore.isFavorite(station) ? "heart.fill" : "heart")
                ...
                .foregroundStyle(libraryStore.isFavorite(station) ? Color.pink : AvtunesysTheme.textInverse.opacity(0.78))
                .frame(width: 36, height: 36)
                .background(Color.white.opacity(0.08), in: Circle())
                .overlay {
                    Circle()
    ...
    private func compactTransportButton(systemImage: String, size: CGFloat, compact: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: systemImage)
                ...
                .foregroundStyle(canCycleStations ? AvtunesysTheme.textInverse : AvtunesysTheme.textInverse.opacity(0.36))
    ...
    private var emptyState: some View {
        VStack(spacing: 18) {
            ...
            Text(L10n.string("player.empty"))
                ...
                .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.84))
    ...
    private func trackTitleLine(for station: Station) -> String {
        if hasPlausibleTrackTitle(for: station),
           let title = AVTunesysText.normalizedValue(audioPlayer.currentTrackTitle) {
            return title
        }
    ...
    private func trackSupportingLine(for station: Station) -> String {
        if hasPlausibleTrackArtist(for: station),
           let artist = AVTunesysText.normalizedValue(audioPlayer.currentTrackArtist) {
            return artist
        }

        if let albumTitle = AVTunesysText.normalizedValue(audioPlayer.currentTrackAlbumTitle) {
            return albumTitle
        }
    ...
    private func hasPlausibleTrackTitle(for station: Station) -> Bool {
        guard AVTunesysText.normalizedValue(audioPlayer.currentTrackTitle) != nil else { return false }
        return !AVTunesysTrackMetadataParser.valueLooksLikeBroadcastMetadata(
    ...
    private func hasPlausibleTrackArtist(for station: Station) -> Bool {
        guard AVTunesysText.normalizedValue(audioPlayer.currentTrackArtist) != nil else { return false }
        return !AVTunesysTrackMetadataParser.artistLooksLikeBroadcastMetadata(
    ...
    private func openExternalSearch(
        ...
        guard let url = AVTunesysExternalSearchURL.url(for: destination, query: query) else { return }
    ...
    private func openStationSearch(for station: Station) {
        guard let url = AVTunesysExternalSearchURL.stationSearch(stationName: station.name) else { return }
        browserDestination = BrowserDestination(url: url)
    }

    private func openArtistSearch(destination: AVTunesysExternalSearchURL.Destination, feature: LimitedFeature) {
        guard hasPlausibleCurrentArtist else { return }
        guard let artist = AVTunesysText.normalizedValue(audioPlayer.currentTrackArtist) else { return }
        guard let url = AVTunesysExternalSearchURL.url(for: destination, query: artist) else { return }
        guard useDailyFeatureIfAllowed(feature, usageKey: url.absoluteString) else { return }
        browserDestination = BrowserDestination(url: url)
    }
...
private struct FlippingPlayerArtwork: View {
    ...
    private var artworkOptionsBack: some View {
                ...
                .fill(
                    LinearGradient(
                        colors: [
                            ...
                            AvtunesysTheme.highlight.opacity(0.32),
        ...
        .shadow(color: AvtunesysTheme.highlight.opacity(0.18), radius: 26, y: 14)
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(songSecondaryLine)
                            ...
                            .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.72))
                            .lineLimit(1)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
                }
    ...
    private var songPrimaryLine: String {
        if let albumTitle = AVTunesysText.normalizedValue(trackAlbumTitle) {
            return albumTitle
        }
...
private enum ArtworkActionStyle {
    ...
    var foreground: Color {
        ...
        case .prominent, .saved:
            return .white
        case .secondary:
            return AvtunesysTheme.textInverse.opacity(0.92)
        }
    }
    ...
    var background: Color {
        switch self {
        case .prominent:
            return AvtunesysTheme.highlight.opacity(0.78)
        case .saved:
