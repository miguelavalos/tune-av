...
struct AuthOnboardingView: View {
    ...
    private let authLogger = Logger(subsystem: "com.avalsys.avtunesys", category: "auth")
            ...
            ZStack {
                ...
                OnboardingBackdrop()
                    .overlay {
                        LinearGradient(
                            colors: [
                                AvtunesysTheme.brandBlack.opacity(0.04),
                                AvtunesysTheme.brandBlack.opacity(authOptionsArePresented ? 0.42 : 0.24),
                                AvtunesysTheme.brandBlack.opacity(0.92)
                            ],
                            startPoint: .top,
                            endPoint: .bottom
                        )
    ...
    private var legalConsentText: AttributedString {
        let termsURL = AppConfig.termsURL?.absoluteString ?? "https://www.avalsys.com/av-account/av-tunesys/terms"
        let privacyURL = AppConfig.privacyURL?.absoluteString ?? "https://www.avalsys.com/av-account/av-tunesys/privacy"
    ...
    private func logAuthError(_ error: Error, provider: String) {
        ...
        authLogger.error(
            "AV Account \(provider, privacy: .public) failed: type=\(String(describing: type(of: error)), privacy: .public) domain=\(nsError.domain, privacy: .public) code=\(nsError.code, privacy: .public) description=\(error.localizedDescription, privacy: .public) underlying=\(underlying, privacy: .public)"
        )
    }
}
        ...
        VStack(spacing: compact ? 14 : 18) {
            ...
            VStack(spacing: compact ? 10 : 12) {
                ...
                Text(L10n.string("auth.feature.subtitle"))
                    ...
                    .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.8))
                    .multilineTextAlignment(.center)
                    .lineLimit(nil)
            ...
            .background(
                RoundedRectangle(cornerRadius: 24, style: .continuous)
                    .fill(AvtunesysTheme.brandBlack.opacity(0.82))
                    .overlay {
                        RoundedRectangle(cornerRadius: 24, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            HStack(spacing: 4) {
                ...
                Text("Tune")
                    .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.92))
...
private struct HeroBadge: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(AvtunesysTheme.brandBlack.opacity(0.8))
            ...
            Circle()
                .fill(
                    RadialGradient(
                        colors: [
                            AvtunesysTheme.highlight.opacity(0.16),
                            .clear
                        ],
                        center: .center,
                        startRadius: 6,
                        ...
                        HStack(spacing: size * 0.04) {
                            ForEach(Array([0.16, 0.34, 0.18].enumerated()), id: \.offset) { index, height in
                                Capsule(style: .continuous)
                                    .fill(index == 1 ? AvtunesysTheme.highlight : Color.white.opacity(0.24))
        ...
        VStack(spacing: 18) {
            Button(action: listenAction) {
                Text(L10n.string("auth.cta.localMode"))
                    ...
                    .foregroundStyle(AvtunesysTheme.brandBlack)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 18)
                    .background(AvtunesysTheme.highlight, in: Capsule())
            }

            Text(accountIsAvailable ? L10n.string("auth.cta.subtitle.available") : L10n.string("auth.cta.subtitle.unavailable"))
                .font(.system(size: 13, weight: .medium))
                .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.76))
                .multilineTextAlignment(.center)
                .lineLimit(2)
            ...
            Button(accountIsAvailable ? L10n.string("auth.cta.continue") : L10n.string("auth.cta.skip"), action: accountIsAvailable ? accountAction : skipAction)
                ...
                .foregroundStyle(AvtunesysTheme.textInverse.opacity(0.88))
        }
        .background(alignment: .top) {
            RadialGradient(
                colors: [
                    AvtunesysTheme.highlight.opacity(0.18),
                    .clear
                ],
                center: .top,
                startRadius: 24,
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 30, style: .continuous)
                .fill(AvtunesysTheme.darkSurfaceAlt.opacity(0.94))
                .overlay {
                    RoundedRectangle(cornerRadius: 30, style: .continuous)
                        .stroke(.white.opacity(0.08), lineWidth: 1)
                }
    ...
    var body: some View {
        GeometryReader { proxy in
            ZStack {
                LinearGradient(
                    colors: [
                        AvtunesysTheme.brandBlack,
                        AvtunesysTheme.darkSurfaceAlt,
                        AvtunesysTheme.brandBlack
                    ],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                ...
                RoundedRectangle(cornerRadius: 42, style: .continuous)
                    .fill(AvtunesysTheme.brandBlack.opacity(0.18))
                ...
                VStack {
                    ...
                    CurvedWave()
                        .stroke(AvtunesysTheme.highlight.opacity(0.1), lineWidth: 2)
                        .frame(height: 180)
    ...
    var body: some View {
        ZStack {
            ForEach([0.56, 0.82, 1.06], id: \.self) { scale in
                Circle()
                    .stroke(AvtunesysTheme.highlight.opacity(scale == 0.82 ? 0.12 : 0.06), lineWidth: 1.5)
            ...
            ForEach([-1.0, 1.0], id: \.self) { direction in
                VStack(spacing: 10) {
                    ForEach([68.0, 96.0], id: \.self) { bar in
                        Capsule(style: .continuous)
                            .fill(AvtunesysTheme.highlight.opacity(0.1))
            ...
            VStack(spacing: 14) {
                ...
                HStack(spacing: 12) {
                    ForEach(Array([0.18, 0.34, 0.22].enumerated()), id: \.offset) { index, height in
                        Capsule(style: .continuous)
                            .fill(index == 2 ? AvtunesysTheme.highlight : Color.white.opacity(0.28))
            ...
            .background(
                RoundedRectangle(cornerRadius: 28, style: .continuous)
                    .fill(AvtunesysTheme.brandBlack.opacity(0.56))
                    .overlay {
                        RoundedRectangle(cornerRadius: 28, style: .continuous)
                            .stroke(Color.white.opacity(0.08), lineWidth: 1)
                    }
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 18) {
            ForEach(Array(heights.enumerated()), id: \.offset) { index, height in
                Capsule(style: .continuous)
                    .fill(index == 1 ? AvtunesysTheme.highlight.opacity(0.08) : Color.white.opacity(0.04))
