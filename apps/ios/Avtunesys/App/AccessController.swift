...
final class AccessController: ObservableObject {
    ...
    private let guestOnboardingLastPromptAtKey = "avtunesys.guestOnboarding.lastPromptAt"
    private let dailyUsagePrefix = "avtunesys.featureUsage."
    ...
    private func limitsWithUITestOverrides(_ resolvedLimits: AccessLimits) -> AccessLimits {
        ...
        guard environment["AVTUNESYS_UI_TESTS"] == "1" else {
            return resolvedLimits
        }

        let favoriteStations = environment["AVTUNESYS_UI_TEST_FAVORITE_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.favoriteStations
        let lyricsSearchesPerDay = environment["AVTUNESYS_UI_TEST_LYRICS_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.lyricsSearchesPerDay
        let webSearchesPerDay = environment["AVTUNESYS_UI_TEST_WEB_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.webSearchesPerDay
        let youtubeSearchesPerDay = environment["AVTUNESYS_UI_TEST_YOUTUBE_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.youtubeSearchesPerDay
        let appleMusicSearchesPerDay = environment["AVTUNESYS_UI_TEST_APPLE_MUSIC_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.appleMusicSearchesPerDay
        let spotifySearchesPerDay = environment["AVTUNESYS_UI_TEST_SPOTIFY_LIMIT"]
            .flatMap(Int.init)
            ?? resolvedLimits.spotifySearchesPerDay
        let discoverySharesPerDay = environment["AVTUNESYS_UI_TEST_DISCOVERY_SHARE_LIMIT"]
            .flatMap(Int.init)
