...
final class DiscoveredTrack {
    ...
    init(record: DiscoveredTrackRecord) {
        let normalizedArtist = AVTunesysDiscoveredTrackSupport.normalizedValue(record.artist)
    ...
    static func makeID(title: String, artist: String?, stationID: String) -> String {
        AVTunesysDiscoveredTrackSupport.makeID(title: title, artist: artist, stationID: stationID, locale: L10n.locale)
    }
