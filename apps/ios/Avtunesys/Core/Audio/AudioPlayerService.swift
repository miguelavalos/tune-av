...
final class AudioPlayerService: NSObject, ObservableObject {
    ...
    private let networkMonitorQueue = DispatchQueue(label: "com.avtunesys.audio-network-monitor")
    ...
    func applyUITestTrackMetadata(title: String?, artist: String?) {
        guard ProcessInfo.processInfo.environment["AVTUNESYS_UI_TESTS"] == "1" else { return }
    ...
    private func updateTrackMetadata(from events: [StreamMetadataEvent]) async {
        ...
        for event in events {
            ...
            if commonKey == "title" || identifier.contains("title") || identifier.contains("streamtitle") {
                let parsed = AVTunesysTrackMetadataParser.parse(value)
            ...
            if commonKey == "artist" || identifier.contains("artist") {
                if let sanitizedArtist = AVTunesysTrackMetadataParser.sanitizeArtist(value) {
        ...
        if AVTunesysTrackMetadataParser.valueLooksLikeBroadcastMetadata(resolvedTitle, stationName: currentStation?.name) {
            resolvedTitle = nil
            resolvedArtist = nil
        } else if AVTunesysTrackMetadataParser.artistLooksLikeBroadcastMetadata(resolvedArtist, stationName: currentStation?.name) {
            resolvedArtist = nil
        }
    ...
    private func applyFallbackTrack(_ track: NowPlayingTrack, for station: Station) {
        ...
        let normalizedArtist = AVTunesysTrackMetadataParser.sanitizeArtist(track.artist)
        guard let normalizedTitle = AVTunesysTrackMetadataParser.sanitizeTitle(track.title, artist: normalizedArtist) else { return }
        guard !AVTunesysTrackMetadataParser.valueLooksLikeBroadcastMetadata(normalizedTitle, stationName: station.name) else { return }

        let resolvedArtist = AVTunesysTrackMetadataParser.artistLooksLikeBroadcastMetadata(normalizedArtist, stationName: station.name)
            ? nil
            : normalizedArtist
    ...
    private func restoreCachedNowPlaying(for station: Station) {
        ...
        let sanitizedArtist = AVTunesysTrackMetadataParser.sanitizeArtist(cachedState.artist)
        let sanitizedTitle = AVTunesysTrackMetadataParser.sanitizeTitle(cachedState.title, artist: sanitizedArtist)
