...
struct KeychainMacAccountTokenProvider: MacAccountTokenProviding {
    static let defaultService = "com.avalsys.avtunesys.mac.av-account"
...
final class AVAccountMacAccessClient: MacAccessProviding {
    ...
    func fetchAccessState() async throws -> MacAccessState {
        ...
        guard let avTunesysAccess = payload.apps.first(where: { $0.appId == "avtunesys" }) else {
...
private enum MacAppDataConstants {
    ...
    static let deviceId = "avtunesys-macos"
}
...
@MainActor
protocol MacAVTunesysLibrarySyncing {
...
final class MacAVTunesysAppDataClient: MacAVTunesysLibrarySyncing {
    private let baseURL: URL?
    private let tokenProvider: () async throws -> String?
    private let urlSession: URLSession
    private let decoder: JSONDecoder
    ...
    private func pullResource<Entry: Codable>(
        ...
        return MacAppDataResourceDocument(
            ...
            updatedAt: AVTunesysDateCoding.date(from: payload.updatedAt),
    ...
    private func pushResource<Entry: Codable>(
        ...
        let envelope = MacAppDataEnvelopePayload(
            ...
            sentAt: AVTunesysDateCoding.string(from: .now),
            entries: entries
        )
...
final class MacAccessController: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard, accessModeKey: String = "avtunesys.mac.accessMode") {
