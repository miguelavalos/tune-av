        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(audioPlayer.isPlaying ? "Live now" : "Paused")
                    .font(.caption)
                    .foregroundStyle(audioPlayer.isPlaying ? AvtunesysTheme.highlight : .secondary)
                    .lineLimit(1)
            }
        }
    }
...
private struct DesktopPlayerInspector: View {
        ...
        .background(
            LinearGradient(
                colors: [
                    ...
                    AvtunesysTheme.neutral100.opacity(0.72)
                ],
                startPoint: .top,
                endPoint: .bottom
            )
            ...
            HStack(alignment: .firstTextBaseline, spacing: 12) {
                ...
                Button {
                    ...
                    Image(systemName: isFavorite(station) ? "heart.fill" : "heart")
                        ...
                        .foregroundStyle(isFavorite(station) ? Color.pink : AvtunesysTheme.textSecondary)
    ...
    private func openStationSearch(for station: Station) {
        if let url = AVTunesysExternalSearchURL.stationSearch(stationName: station.name) {
            openURL(url)
        }
    }
...
private struct DesktopPlayerArtwork: View {
    ...
    private func artworkActionButton(systemImage: String, title: String, isProminent: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .foregroundStyle(isProminent ? Color.black : AvtunesysTheme.textInverse)
            .frame(maxWidth: .infinity)
            .frame(height: 42)
            .background(isProminent ? AvtunesysTheme.highlight : Color.white.opacity(0.12), in: RoundedRectangle(cornerRadius: 12, style: .continuous))
            .overlay {
    ...
    private func artworkIconButton(systemImage: String, title: String, isProminent: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .frame(maxWidth: .infinity)
            .frame(height: 58)
            .background(isProminent ? AvtunesysTheme.highlight : Color.white.opacity(0.12), in: RoundedRectangle(cornerRadius: 14, style: .continuous))
            .overlay {
...
private struct PlayerActionButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .foregroundStyle(isActive ? Color.white : AvtunesysTheme.textPrimary)
            .frame(maxWidth: .infinity, minHeight: 58)
            .background(isActive ? AvtunesysTheme.highlight : AvtunesysTheme.elevatedSurface, in: RoundedRectangle(cornerRadius: 16, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(isActive ? Color.clear : AvtunesysTheme.borderSubtle, lineWidth: 1)
            }
        }
        .buttonStyle(.plain)
    }
