...
final class LibraryStoreSnapshotTests: XCTestCase {
    ...
    func testDiscoveryShareTextFormatterBuildsLimitedVisibleList() {
        ...
        XCTAssertEqual(lines.first, "AV Tunesys discoveries")
    ...
    func testAccessControllerResolvesBackendReadyStateByMode() {
        ...
        XCTAssertEqual(defaults.string(forKey: "avtunesys.mac.accessMode"), "signedInPro")
    }
    ...
    func testAccessClientRequiresAVTunesysAppAccessFromBackendPayload() async {
        let client = AVAccountMacAccessClient(
            baseURL: URL(string: "https://api.example.com")!,
            tokenProvider: { "token" },
            urlSession: mockURLSession(statusCode: 200, body: Self.missingAVTunesysAccessResponseJSON)
        )

        do {
            _ = try await client.fetchAccessState()
            XCTFail("Expected missing AV Tunesys access to fail")
    ...
    func testAppDataClientPullsLibraryFromResourceDocuments() async throws {
        ...
        let client = MacAVTunesysAppDataClient(
            baseURL: URL(string: "https://api.example.com")!,
            tokenProvider: { "token" },
        ...
        XCTAssertEqual(requestedPaths, [
            "/v1/apps/avtunesys/data/favorites",
            "/v1/apps/avtunesys/data/recents",
            "/v1/apps/avtunesys/data/discoveries",
            "/v1/apps/avtunesys/data/settings"
        ])
    ...
    func testAppDataClientPushesLibraryAsSeparateResourceDocuments() async throws {
        ...
        XCTAssertEqual(pushedDeviceIDs, Array(repeating: "avtunesys-macos", count: 4))
    }
    ...
    func testAppDataClientThrowsSyncConflictWhenResourceRetryStillConflicts() async throws {
        ...
        do {
            ...
            XCTFail("Expected AVTunesysAppDataError.conflict, got \(error).")
        }
    }
    ...
    func testCloudSyncAppliesRemoteSnapshotForProAccess() async {
        ...
        store.updateAccessMode(.signedInPro)
        let remoteUpdatedAt = AVTunesysDateCoding.string(from: Date(timeIntervalSinceNow: 60))
        let syncClient = MockMacLibrarySyncClient(
            remoteDocument: AVTunesysLibraryDocument(
    ...
    func testCloudSyncConflictSetsConflictStatus() async {
        ...
        let syncClient = MockMacLibrarySyncClient(
            ...
            pushError: AVTunesysAppDataError.conflict
        )
    ...
    func testReplaceLocalLibraryWithCloudDataResolvesConflictFromCloud() async {
        ...
        let syncClient = MockMacLibrarySyncClient(
            remoteDocument: AVTunesysLibraryDocument(
                snapshot: testSnapshot(settingsUpdatedAt: AVTunesysDateCoding.string(from: remoteUpdatedAt)),
    ...
    func testCloudAppliedSnapshotUsesStableUpdatedAtForGeneratedRecordDates() async {
        let store = LibraryStore(defaults: isolatedUserDefaults())
        store.updateAccessMode(.signedInPro)
        let remoteUpdatedAt = AVTunesysDateCoding.date(from: "2026-05-01T10:00:00.000Z")
        let syncClient = MockMacLibrarySyncClient(
            remoteDocument: AVTunesysLibraryDocument(
                snapshot: AVTunesysLibrarySnapshot(
                    favorites: [
    ...
    func testPreferenceChangesAdvanceLocalSnapshotTimestampForSyncPlanning() async {
        ...
        let syncClient = MockMacLibrarySyncClient(
            remoteDocument: AVTunesysLibraryDocument(
                ...
                updatedAt: AVTunesysDateCoding.date(from: remoteUpdatedAtString),
                revision: 2,
                etag: nil
            )
        )
    ...
    func testBackendBootstrapHandlesMissingAVTunesysAccessAsRefreshFailure() async {
        let store = LibraryStore(defaults: isolatedUserDefaults())
        ...
        XCTAssertEqual(store.backendConnectionStatus, .accessRefreshFailed)
        XCTAssertEqual(store.accountConnectionState, .accessRefreshFailed)
        XCTAssertEqual(store.backendConnectionFailureTitle, "AV Tunesys access missing")
        XCTAssertTrue(store.canRetryBackendConnection)
        XCTAssertFalse(store.isCloudSyncConfigured)
        XCTAssertFalse(store.canRunCloudSync)
        XCTAssertEqual(store.cloudSyncReadinessTitle, "Access refresh failed")
    ...
    func testLoadingGuestStateTrimsPersistedSavedTracksToLimit() {
        ...
        defaults.set("guest", forKey: "avtunesys.mac.accessMode")
    ...
    private static func appDataResourceResponse(resource: String) -> (HTTPURLResponse, Data) {
        ...
        """
        let response = HTTPURLResponse(
            url: URL(string: "https://api.example.com/v1/apps/avtunesys/data/\(resource)")!,
            statusCode: 200,
            httpVersion: nil,
            headerFields: ["Content-Type": "application/json"]
        )!
    ...
    private static let missingAVTunesysAccessResponseJSON = """
    {
      "viewer": {
        "isAuthenticated": true,
        "userId": "user_123",
    ...
    private func testSnapshot(settingsUpdatedAt: String = "2026-04-30T12:00:00.000Z") -> AVTunesysLibrarySnapshot {
...
@MainActor
private final class BlockingMacLibrarySyncClient: MacAVTunesysLibrarySyncing {
    private let remoteDocument: AVTunesysLibraryDocument
    private var pullContinuation: CheckedContinuation<AVTunesysLibraryDocument, Never>?
    private(set) var pullCount = 0

    init(remoteDocument: AVTunesysLibraryDocument) {
...
@MainActor
private final class MockMacLibrarySyncClient: MacAVTunesysLibrarySyncing {
    var pullCount = 0
    var pushCount = 0
    var overwriteCount = 0
    var pushedSnapshots: [AVTunesysLibrarySnapshot] = []
