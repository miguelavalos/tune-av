...
final class AudioPlayerService: ObservableObject {
    ...
    func play(_ station: Station) {
        guard let url = URL(string: station.streamURL) else {
            failPlayback(L10n.string("mac.audio.invalidStreamURL"))
            return
        }
    ...
    private func attachObservers(to player: AVPlayer, item: AVPlayerItem) {
        statusObservation = item.observe(\.status, options: [.initial, .new]) { [weak self] item, _ in
            Task { @MainActor in
                ...
                @unknown default:
                    self.failPlayback(L10n.string("mac.audio.unknownPlaybackState"))
                }
            }
        }
        ...
        loadingTimeoutTask = Task { [weak self] in
            ...
            await MainActor.run {
                ...
                if case .loading = self.playbackState {
                    self.failPlayback(L10n.string("mac.audio.startTimeout"))
                }
            }
        }
    }
    ...
    private func playbackErrorMessage(from error: Error?) -> String {
        ...
        if nsError?.domain == NSURLErrorDomain, nsError?.code == NSURLErrorAppTransportSecurityRequiresSecureConnection {
            return L10n.string("mac.audio.insecureStream")
        }
        return error?.localizedDescription ?? L10n.string("mac.audio.startFailed")
    }
