...
usage() {
  cat <<'USAGE'
Usage:
  scripts/check-macos-release-config-hygiene.sh [--env dev|prod] [--configuration Debug|Release]

Checks that macOS Local.xcconfig is present only as an ignored local artifact,
...
local_config="apps/macos/Config/Local.xcconfig"
...
"$repo_root/scripts/check-macos-runtime-config.sh" --env "$env_name" --configuration "$configuration"

echo "macOS release config hygiene check passed."
